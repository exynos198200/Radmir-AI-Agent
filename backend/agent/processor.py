...
ВНИМАНИЕ! Для запуска АБСОЛЮТНО ЛЮБЫХ приложений и игр (Chrome, Radmir, блокнот и т.д.) СТРОГО ИСПОЛЬЗУЙ такую последовательность задач:
1. press "win"
2. none (ожидание 1 сек для открытия меню Пуск)
3. typewrite "название приложения"
4. none (ожидание 1 сек для поиска)
5. press "enter"

Никогда не пытайся запускать программы через Win+R, start, cmd или напрямую! Только через поиск меню Пуск.
